...
- **No topic given:** you are the editor — pick this week's subject yourself.
  ...
  2. **An evergreen subject** — a durable question practitioners keep getting
     ...
     **evergreen shelf**; the guide's own thin sections are the other source.
     Grep `editorial/COVERAGE.md` for what has gone longest uncovered.
...
Always grep `editorial/COVERAGE.md` first: don't repeat a past dive unless the
...
what's left open) and attach the dive to its running thread. Do not hand-append
coverage lines — `editorial/COVERAGE.md` is regenerated by the gates.
...
shelf is down to its last two, add two. Draw them from the guide sections
`editorial/COVERAGE.md` shows as thinnest. Leaving the shelf empty hands next
Thursday's run nothing to pick from.
